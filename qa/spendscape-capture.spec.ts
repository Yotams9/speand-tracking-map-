...
test('Capture browser history, barcode boundary, Gmail explanation, manual validation, failure retry, and unresolved zero-pin behavior work', async ({ browser }) => {
  ...
  await page.getByTestId('capture-confirm').click()
  expect((await evidence(page)).sessionPurchaseCount).toBe(0)
  await page.getByTestId('review-placeId').selectOption('place_shuk_bograshov')
  await page.getByTestId('review-amount').fill('12.9')
  await expect(page.getByTestId('capture-confirm')).toBeEnabled()
  await expect(page.getByTestId('capture-review')).toContainText('Product photos are not retained')
  await page.getByTestId('capture-dialog').getByRole('button', { name: 'Close Capture' }).click()
  ...
  await page.getByTestId('review-channel').selectOption('unknown')
  await page.getByTestId('review-amount').fill('0')
  await page.getByTestId('manual-review').click()
  await expect(page.locator('#review-error-amount')).toContainText('positive decimal amount')
  await page.getByTestId('review-amount').fill('41')
  await page.getByTestId('review-merchantId').selectOption('merchant_unresolved')
  await page.getByTestId('review-date').fill('2026-08-29T17:45')
